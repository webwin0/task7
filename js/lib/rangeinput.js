	...
	function RangeInput(input, conf) {
		...
		$.extend(self, {  
			...
			getCaretPosition: function  () {
				...
				// Firefox support
				else if (input.get(0).selectionStart || input.get(0).selectionStart == '0')
					CaretPos = input.get(0).selectionStart;
				return (CaretPos);
			},
			...
			step: function(am, e) {
				...
				var caretPositin = self.getCaretPosition();
				self.setValue(value + step * (am || 1), e);	
				self.setCaretPosition(caretPositin);
			},
		...
		if (conf.keyboard) {
			...
			input.keydown(function(e) {
				...
				var key = e.keyCode,
					 up = $([75, 76, 38, 33]).index(key) != -1, //, 39
					 down = $([74, 72, 40, 34]).index(key) != -1; //, 37					 
